...
add_action('wp_ajax_get_timeline_posts', 'get_timelins_posts');
add_action('wp_ajax_nopriv_get_timeline_posts', 'get_timelins_posts');

function get_ajax_posts() {
	// Query Arguments
	global $post;
	$cat = $_REQUEST['cat_id'];
	$order = $_REQUEST['post_sort'] ;
	$view = $_REQUEST['time_view'];
	$args = array(
		'posts_per_page' => 40,
		'nopaging' => true,
		'order' => $order,
	...
	// The Query
	$allposts = get_posts( $args ); // changed to get_posts from wp_query, because `get_posts` returns an array
	$output = '<div class = "row">';
	if ( $view == 'timeline' ) {
		$arr = displayTimelineLinks($allposts);
	$output .= $arr;
	 } else {
	$output .= displayHTMLView($allposts);
	}
	$output .= '</div>';
...
function displayHTMLView($posts){
	global $post;
	foreach( $posts as $post ): setup_postdata( $post );
  ...
  // print_r($post);
	  ...
	  $output .= '<div class = "card-header bg-1"><h3>'.$post->post_title.' </h3></div>';
...
add_shortcode('get_latest_news','getLatestPosts');
...
if (function_exists('register_sidebar'))
{
	...
	register_sidebar(array(
		'name' => __('UPS seneste nyheder', 'html5blank'),
	...
	register_sidebar(array(
		'name' => __('UPS sponsorer', 'html5blank'),
		'description' => __('Description for this widget-area...', 'html5blank'),
		'id' => 'widget-area-2',
		'before_widget' => '<div id="%1$s" class="%2$s">',
		'after_widget' => '</div>',
...
{
	register_taxonomy_for_object_type('category', 'custom-page'); // Register Taxonomies for Category
	register_taxonomy_for_object_type('post_tag', 'custom-page');
	register_post_type('html5-blank', // Register Custom Post Type
		array(
		'labels' => array(
			'name' => __('Projekt Custom Pages', 'projekt custom page'), // Rename these to suit
			'singular_name' => __('Projekt Custom Pages', 'projekt custom page'),
			'add_new' => __('Add New', 'Custom page'),
			'add_new_item' => __('Custom Page Post', 'custom page'),
			'edit' => __('Edit', 'custom page'),
			'edit_item' => __('Edit Custom Page', 'custom page'),
			'new_item' => __('New Custom Page', 'custompage'),
			'view' => __('View Custom Page', 'custompage'),
			'view_item' => __('View Custom Page', 'custompage'),
			'search_items' => __('Search Custom Page', 'custompage'),
			'not_found' => __('No Custom Page found', 'custompage'),
			'not_found_in_trash' => __('No Custom Page in Trash', 'custompage')
		),
		'public' => true,
...
function my_tiny_mce_before_init( $in ) {
	...
	$in['theme_advanced_buttons2'] = 'formatselect,pastetext,pasteword,charmap,undo,redo,postsselect';
...
{
	return '<div class="shortcode-demo">' . do_shortcode('test') . '</div>'; // do_shortcode allows for nested Shortcodes
}
